...
def shap_explain_global_ransome(cnn_model, X_background, X_test, id2token, top_n=5):
    ...
    plot_top_shap_bar(top_pos + top_neg)  # hoặc tách riêng nếu muốn

    return top_pos+top_neg

def shap_explain_global_benign(cnn_model, X_background, X_test, id2token, top_n=5):
    try:
        explainer = shap.GradientExplainer(cnn_model, X_background)
        shap_vals = explainer.shap_values(X_test)
    except Exception:
...
def plot_top_shap_bar(top_tokens):
    ...
    plt.title('Global SHAP Values for Ransomware Prediction', fontsize=14)
